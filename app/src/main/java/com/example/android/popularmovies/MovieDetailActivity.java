...
import static com.example.android.popularmovies.utility.NetworkUtils.createUrl;
import static com.example.android.popularmovies.utility.NetworkUtils.extractReviewFromJson;
import static com.example.android.popularmovies.utility.NetworkUtils.extractTrailerKeyFromJson;
import static com.example.android.popularmovies.utility.NetworkUtils.getResponseFromHttpUrl;
...
public class MovieDetailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (intentFromMainActivity.hasExtra("id")) {
            ...
            TrailerAsyncTask taskForTrailer = new TrailerAsyncTask();
            ReviewAsyncTask taskForReview = new ReviewAsyncTask();
            taskForTrailer.execute();
    ...
    private class TrailerAsyncTask extends AsyncTask<URL, Void, ArrayList<Trailer>> {
        ...
        protected ArrayList<Trailer> doInBackground(URL... urls) {
            ...
            String jsonResponse = " ";
            try {
                jsonResponse = getResponseFromHttpUrl(trailerUrl);
            } catch (IOException e) {
                e.printStackTrace();
            }
            ArrayList<Trailer> trailers = extractTrailerKeyFromJson(jsonResponse);
            return trailers;
        }
        ...
        protected void onPostExecute(ArrayList<Trailer> trailers) {
            mTrailers = trailers;
            TrailerAdapter adapter = new TrailerAdapter(getApplicationContext(), mTrailers);
            trailerListView = (RecyclerView) findViewById(R.id.trailer_recycler_view);
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        if (mTrailers != null && mTrailers.size() > 0) {
            myShareIntent.putExtra(Intent.EXTRA_TEXT, "https://www.youtube.com/watch?v=" + mTrailers.get(0).getTrailerKey());
        }
